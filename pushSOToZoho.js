...
async function createOrUpdateSO(
  ...
  // // Use SKU from existingSO if available, otherwise from soDetails
  // const skuToUse = existingSO?.line_items?.[0]?.sku || soDetails.sku;

  // try {
  //   const fetchedItem =
  //     (await fetchItemFromZoho(skuToUse, authToken, sendEmail)) || {};
  //   itemId = fetchedItem.itemId || null;
  //   itemInitialStock = fetchedItem.itemInitialStock || null;

  //   if (!itemId) console.warn(`No Item ID found for SKU ${skuToUse}`);
  //   if (itemInitialStock === undefined)
  //     console.warn(`Initial stock not available for SKU ${skuToUse}`);
  // } catch (error) {
  //   console.error(
  //     `Error fetching item details for SKU ${skuToUse}:`,
  //     error.message,
  //   );
  // }
